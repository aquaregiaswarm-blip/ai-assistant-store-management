...
async def get_kpis(
    ...
    return {
        ...
        "peak_hour": int(throughput["peak_hour"]) if throughput else 15,
        "peak_transactions": throughput["peak_transactions"] if throughput else 0,
...
async def get_alerts(
    ...
    for v in violations:
        ...
        alerts.append({
            ...
            "message": f"{v['First_Name']} {v['Last_Name'][0]}. - {v['Description']}",
            "employee": f"{v['First_Name']} {v['Last_Name'][0]}."
        })
    ...
    for m in minor_hours:
        alerts.append({
            ...
            "message": f"{m['First_Name']} {m['Last_Name'][0]}. ({m['Actual_Hours']:.1f} hrs) needs a break soon",
            "employee": f"{m['First_Name']} {m['Last_Name'][0]}."
        })
